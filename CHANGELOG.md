...
#### [0.4.0](https://github.com/vetalkordyak/AirCon/compare/0.3.17...0.4.0)

> 29 August 2026

- Replace the MQTT bridge with a native ESPHome API server (via `aioesphomeserver`), removing the broker dependency and its silent-disconnect failure mode.
- Expose swing mode, humidity/voltage sensors, filter-clean status, and backlight/eco/quiet-mode switches over the ESPHome API, in addition to the core climate entity (power/mode/temperature/fan speed).
- Add a Docker `HEALTHCHECK` that performs a real ESPHome handshake, plus an `autoheal` sidecar in `docker-compose.yaml` to recover automatically if `aioesphomeserver`'s connection handling wedges (a known rough edge in that alpha-quality dependency).
- Add `esphome_advertise_ip` to fix Home Assistant reporting the device unavailable when it can only reach this server through a different address than the one it would pick on its own (e.g. a Docker macvlan/bridge split).
- Bump the Docker base image to Python 3.14 (required by `aioesphomeserver`).
- Update the `hassio` add-on packaging (`config.json`, `DOCS.md`), which still described an MQTT broker dependency and pointed at upstream's un-forked Docker Hub image.
- Remove the SmartThings groovy device handler and its docs - SmartThings retired the classic Groovy IDE/Device Handlers platform at the end of 2022.
- Remove `new_version.sh`, upstream's release script for pushing multi-arch builds to `deiger/aircon` on Docker Hub - not applicable to this fork.
