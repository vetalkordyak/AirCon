...
Це форк [deiger/AirCon](https://github.com/deiger/AirCon). Оригінал підключає кондиціонер(и) до
Home Assistant через MQTT-брокер; цей форк повністю прибирає MQTT і замість цього піднімає
нативний сервер [ESPHome] API (через [aioesphomeserver]) — кондиціонер(и) з'являються напряму в
інтеграції ESPHome Home Assistant, тій самій, якою він уже користується для справжніх
ESPHome-пристроїв. Без брокера, з push-оновленнями стану та обробкою перепідключень, яку вже
безкоштовно надає ESPHome-клієнт, що йде разом з Home Assistant. Детальніше про це і про поточні
обмеження — у розділі [Інтеграція з ESPHome](#інтеграція-з-esphome) нижче.
...
1. Перевірте, що сервер запускається, наприклад:
   ...
   - `--esphome_advertise_ip` — IP-адреса, під якою пристрій анонсується через Zeroconf/mDNS, якщо
     вона відрізняється від тієї, яку ця машина обрала б сама. Див.
     [Інтеграція з ESPHome](#інтеграція-з-esphome) нижче.
...
Кожен налаштований кондиціонер з'являється в Home Assistant як пристрій з такими сутностями:

| Сутність                  | Тип             | Властивість Hisense                                    | Примітка |
|----------------------------|-----------------|----------------------------------------------------------|----------|
| *(назва кондиціонера)*     | `climate`       | `t_power`, `t_work_mode`, `t_temp`, `f_temp_in`, `t_fan_speed`, `t_fan_power`, `t_fan_leftright` | Режими HVAC off/heat/cool/dry/fan_only/auto; швидкість вентилятора представлена як *custom fan mode* з власними назвами кондиціонера (`auto`/`lower`/`low`/`medium`/`high`/`higher`), а не втратно мапиться на фіксований набір ESPHome; swing (off/vertical/horizontal/both) керує одразу вертикальною і горизонтальною заслінками |
| … Backlight                | `switch`        | `t_backlight`                                            | підсвітка дисплея |
| … Eco Mode                 | `switch`        | `t_eco`                                                  | |
| … Quiet Mode                | `switch`        | `t_fan_mute`                                             | |
| … Filter Clean Required    | `binary_sensor` | `f_filterclean`                                          | |
| … Humidity                 | `sensor`        | `f_humidity`                                             | на деяких моделях завжди повертає 0 — це особливість самого кондиціонера, не інтеграції |
| … Voltage                  | `sensor`        | `f_voltage`                                              | те саме, що й з Humidity |

Ще не представлено: режим сну (`t_sleep`), швидкі режими нагріву/охолодження (`t_temp_heatcold`,
`t_temp_eight`) та сирі коди помилок (`f_e_*`). Тим часом користуйтесь безпосередньо
...
[HTTP API](#запуск-сервера-керування-кондиціонером-вручну) для стандартних кондиціонерів
